<script setup>
    ...
    const savePosition = ()=>{
        ...
        if(form.id == undefined){
            axios.post('api/leave-type', form).then((res)=>{
                toast.fire({
                    icon:'success',
            ...
            axios.put('api/leave-type/'+form.id, form).then((res)=>{
                toast.fire({
                    icon:'success',
    ...
    const getData = ()=>{
        axios.get('/api/leave-type?search='+searchData.value).then((res)=>{
    ...
    const deleteData = (data)=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/api/leave-type/${data.id}`).then((res)=>{
                    toast.fire({
                        title: "Deleted!",
...
<template>
    <div class="container">
        <h4 class="mt-4">LEAVE TYPE SETUP</h4>
                    ...
                    <table class="table table-sm table-striped">
                        ...
                        <tbody>
                            <tr v-for="(list, index) in listData" :key="index">
                                ...
                                <td class="text-muted">
                                    <div class="btn-group" v-if="filterLeaveType(list)">
